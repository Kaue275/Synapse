...
if selected == "Pesquisar":
    col1 = st.columns([1])
    with col1:
        # Adicione uma barra de pesquisa na barra lateral
        search_query = st.sidebar.text_input('Pesquisar')

        # Filtrar os dados com base na consulta de pesquisa
        filtered_data = dfs_cidades[cidade_selecionada][dfs_cidades[cidade_selecionada]['Métricas'].str.contains(search_query, case=False)]
